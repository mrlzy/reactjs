            ...
            <div class="col-sm-3">
                <div class="widget-box widget-color-blue2">
                    <div class="widget-header">
                        <h4 class="widget-title lighter smaller">菜单组织</h4>
                    </div>
    ...
    jQuery(function($) {
        ...
        jQuery(grid_selector).jqGrid({
            ...
            subGridRowExpanded: function (subgridDivId, rowId) {
                ...
                var subgridSelectorId = subgridDivId + "_p";


                $("#" + subgridDivId).html("<table id='" + subgridTableId + "'></table>  <div id='"+subgridSelectorId+"'></div>");


                var  subgridSelector='#'+subgridSelectorId;

                $("#" + subgridTableId).jqGrid({
                    datatype: 'json',
                    mtype: 'POST',
                    url: 'perm/listPermForMenu?menu_id='+rowId,
                    jsonReader: {
                        id:"perm_id",
                    ...
                    pager : subgridSelector,
                    rowNum:3,
                    rowList:[3,5,10],
                    viewrecords : true,
                    altRows: true,
                    multiselect: true,
                    colNames: ['授权ID','操作码','操作说明'],
                    colModel: [
                        { name: 'perm_id',index:'perm_id',sortable:false, width: 250 ,editable: true, align:"center",editoptions:{size:"20",maxlength:"20",readonly:true}},
                        { name: 'perm_code',index:'perm_code',sortable:false, width: 120,editable: true,editoptions:{size:"20",maxlength:"20"},align:"center",
                            editrules:{required:true,custom:true,custom_func:function (val, nm) {
                                if(/^[a-z][a-z|\*]*$/.test(val)||val=='*'){
                                    return [true,""];
                                }
                                return[false,nm+':只能包含[a-z]和*,且不能以*开头带字母'];

                            }}
                        },
                        { name: 'remark',index:'remark',sortable:false, width: 230,editable: true, edittype:"textarea", editoptions:{rows:"3",cols:"25"},align:"center"}
                    ],
                    ...
                    editurl: "perm/opPerm?menu_id="+rowId,
                    caption: "菜单控制"
                });


                jQuery("#" + subgridTableId).jqGrid('navGrid',subgridSelector,
                        { 	//navbar options
                            edit: true,
            ...
            height: 250,
            colNames:['菜单编号','菜单名称','上级编号','上级菜单','请求地址','css','排序号','状态','叶子节点','备注'],
            colModel:[
                {name:'menu_id',index:'menu_id', width:80, sorttype:"int", editable: true, hidden :true, align:"center"},
                {name:'menu_name',index:'menu_name', width:80, search:true, searchoptions:{sopt:["eq","ne","cn","nc","bw","bn","ew","en"]},   editable: true,editoptions:{size:"20",maxlength:"30"},editrules:{required:true},align:"center"	},
                ...
                {name:'url',index:'url', search:false,width:80,editable: true,editoptions:{size:"20",maxlength:"30"},align:"center"},
                {name:'icon_cls',index:'icon_cls',search:false, width:120,editable: true,editoptions:{size:"20",maxlength:"30"},align:"center"},
                ...
                {name:'statu',index:'statu', search:true,stype:'select',  searchoptions:{sopt:["eq"],value:{0:"无效",1:"有效"}},   width:80, editable: true,edittype:"select",editoptions:{value:"0:无效;1:有效"},formatter:function(cellvalue){if(cellvalue==0)return '无效';return '有效';}, align:"center"},
            ...
            editurl: "menu/opMenu",//nothing is saved
            caption: "菜单列表"
        ...
        jQuery(grid_selector).jqGrid('navGrid',pager_selector,
                ...
                },
                {
                    ...
                    afterSubmit:function (response, postdata){
                        var obj =eval('('+response.responseText+')');
                        if(obj.success){
                              $('#itme_'+postdata.menu_id).text(postdata.menu_name);
                        }
                        return [obj.success,obj.msg];
                    },
                    beforeShowForm : function(e) {
            ...
            select: function(event, ui) {
                ...
                switch(ui.cmd){
                    ...
                    case "new":
                        ...
                        break;
                    case "delete":
                        $(grid_selector).jqGrid("delGridRow",$target.closest("li[role='treeitem']").data().id,{
                            recreateForm: true,
                            serializeDelData:function(postd){
                                postd['menu_id']=postd.id;
                                return postd;
                            },
                            afterSubmit:function (response, postdata){
                                var obj =eval('('+response.responseText+')');
                                if(obj.success){
                                    var $ptarget=$target.closest('.tree-branch-children').prev().find('.tree-label');

                                    if($ptarget.length){
                                        $('#tree1').tree("openFolder",$ptarget);
                                        $ptarget.closest('.tree-branch').find('.tree-branch-children').empty(); // tree branch
                                        $('#tree1').tree("openFolder",$ptarget);
                                    }else{
                                        $('#tree1').data("fu.tree",null);
                                        $('#tree1').off("click");
                            ...
                            beforeShowForm : function(e) {
                                ...
                                form.find('.delmsg').text('你确定永久删除['+$target.text()+']菜单?');
                                style_delete_form(form);
