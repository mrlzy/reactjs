<div class="page-content">
    ...
    <div class="row">
            <div class="col-sm-3">
                <div class="widget-box widget-color-blue2">
                    <div class="widget-header">
                        <h4 class="widget-title lighter smaller">角色组织</h4>
                    </div>
        ...
        <div  id="menu_win" class="col-sm-6 widget-container-col" style=" position: absolute;  z-index: 4444;float: right;right: 200px;">

            <div class="widget-box">
                <div class="widget-header">
                    <h4 class="widget-title lighter">请选择菜单权限</h4>

                    <div class="widget-toolbar no-border">
                        <ul class="nav nav-tabs" id="myTab2">
                            <li class="active">
                                <a data-toggle="tab" href="#home2">MenuPerm</a>
                            </li>
                ...
                <div class="widget-body">
                    ...
                    <div class="widget-toolbox padding-8 clearfix">
                        <button class="btn btn-xs btn-danger pull-right"  onclick=" $('#menu_win').hide()">
                            <i class="ace-icon fa fa-times"></i>
                            <span class="bigger-110">关闭</span>
                        </button>

                        <button class="btn btn-xs btn-success pull-left" onclick="subMenuRole()" >
...
<script type="text/javascript">
     var menu_win='menu_win';
     var roleId="";
     var perms=[];
     var permsf=[];


   function isSelectPerms(p){
     ...
     function showMenuRoleWin(id){
         var $sels =$('#'+menu_win).find('.tree-selected');
         $sels.removeClass('tree-selected');
         roleId=id;

        $.ajax({
             url: 'role/selectPerms',
             data: {'role_id':roleId},
             type: 'POST',
             dataType: 'json',
             success : function(result) {
                 if(!result.success){
                     ...
                     perms=result.perms;
                     permsf.length=perms.length;
                     for(var i=0;i<permsf.length;i++){
                         permsf[i]=0;
                     }
                     listenMenuRole();
                     $('#'+menu_win).show();
     ...
     function listenMenuRole(){
         var $sels =$('#tree2').find('.tree-item-name > .tree-label');
         $.each($sels, function (index, value) {
             var dd=$(value).attr("id")||'';
             var id=dd.substring(5);
             if(isSelectPerms(id)){
                 $('#'+dd).parent().parent().addClass('tree-selected');
             }
     ...
     function subMenuRole(){
         ...
         $.ajax({
             url: 'role/authorizePerm',
             data: {'role_id':roleId,'ids':ids},
             type: 'POST',
             dataType: 'json',
             success : function(result) {
                 if(!result.success){
    ...
    jQuery(function($) {
        ...
        jQuery(grid_selector).jqGrid({
          ...
          /*  subGridRowExpanded: function (subgridDivId, rowId) {
            ...
            height: 250,
            colNames:['角色编号','角色名称','上级编号','上级角色','排序号','叶子节点','备注'],
            colModel:[
                {name:'role_id',index:'role_id', width:80, sorttype:"int", editable: true, hidden :true, align:"center"},
                {name:'role_name',index:'role_name', width:80, search:true, searchoptions:{sopt:["eq","ne","cn","nc","bw","bn","ew","en"]},   editable: true,editoptions:{size:"20",maxlength:"30"},editrules:{required:true},align:"center"	},
            ...
            editurl: "role/opRole",//nothing is saved
            caption: "角色列表"
        ...
        jQuery(grid_selector).jqGrid('navGrid',pager_selector,
                ...
                },
                {
                    ...
                    afterSubmit:function (response, postdata){
                        ...
                        if(obj.success){
                              $('#itme_'+postdata.role_id).text(postdata.role_name);
                        }
        ...
        var remoteUrl = 'role/treeComponentRole';
            ...
            select: function(event, ui) {
                ...
                switch(ui.cmd){
                    ...
                    case "new":
                        ...
                        break;
                    case "read_menu":
                        showMenuRoleWin($target.attr('id'));
                        break;
                    case "cut":
                    case "copy":
        ...
        var remoteUrl2 = 'perm/treeComponentMenuPerm';
