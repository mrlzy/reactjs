...
(function (factory) {
	...
	var Tree = function (element, options) {
		...
		if( this.options.folderSelect ){
			...
			this.$element.on('click.fu.tree', '.icon-folder ', $.proxy( function(ev) { this.openFolder($(ev.currentTarget).parent()); }, this));
			this.$element.on('click.fu.tree', '.ace', $.proxy( function(ev) { this.selectFolder($(ev.currentTarget)); }, this));
		}
	...
	Tree.prototype = {
			...
			this.options.dataSource( treeData ? treeData : {} , function (items) {
				...
				$.each( items.data, function(index, value) {
					...
					if(value.type === 'folder') {
						...
						$entity.find('.tree-branch-name > .tree-label').attr("id","itme_"+value.id).html((value.text || value.name));
						$entity.find('.tree-branch-name > .ace').attr("id","folder_"+value.id);
						...
						if('item-selected' in value && value['item-selected']==true){
							//$entity.find('input.ace').click();
							self.selectFolder($entity.find('input.ace'));
						}
						
						
					} else if (value.type === 'item') {
						$entity = self.$element.find('[data-template=treeitem]:eq(0)').clone().removeClass('hide').removeAttr('data-template');
						$entity.find('.tree-item-name > .tree-label').attr("id","itme_"+value.id).html((value.text || value.name));
						$entity.data(value);
		...
		/*	if (!this.options.folderSelect) {
			...
			//alert('.'+$.trim(this.options['close-icon'].replace(/(\s+)/g, '.')));
			//alert($el.attr("class"));
			//alert($el.find('.'+$.trim(this.options['close-icon'].replace(/(\s+)/g, '.'))).length);

			var eventType, classToTarget, classToAdd;
			if ($el.children('div .tree-branch-header').find('.'+$.trim(this.options['close-icon'].replace(/(\s+)/g, '.'))).length) {//ACE
